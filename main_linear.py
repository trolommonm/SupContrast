...
def check_mean_per_class_ds(opt):
    return opt.dataset in MEAN_PER_CLASS_DATASETS
...
def parse_option():
    ...
    parser.add_argument('--augmentation', type=str, default='none',
                        choices=['autoaugment', 'randaugment', 'simaugment', 'none', 'finetune'], help='choose augmentation')
...
def set_model(opt):
    ...
    if torch.cuda.is_available():
        if torch.cuda.device_count() > 1 and not opt.no_data_parallel:
...
def main():
    ...
    # build data loader
    train_loader, val_loader = set_loader(opt, "linear")
